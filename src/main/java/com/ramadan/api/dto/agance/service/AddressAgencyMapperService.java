...
public class AddressAgencyMapperService implements IAddressAgencyMapperService{
    ...
    @Override
    public Page<AddressAgencyResponseDto> convertToResponseDto(Page<Address> addresses) {
        return addresses.map((element) -> modelMapper.map(element, AddressAgencyResponseDto.class));
    }
    ...
    public AddressAgencyResponseDto convertEntityToDto(AdresseAgency oAdresseAgency) {
        modelMapper.getConfiguration().setMatchingStrategy(MatchingStrategies.STANDARD);
        return modelMapper.map(oAdresseAgency, AddressAgencyResponseDto.class);
    }
